...
exports.youtubeCrawlingFunction = async (cb) => {

    const url = "https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=1&ie=utf8&query=auz";
    ...
    (async() => {
        ...
        await page.waitForSelector('#wrap > #container > #content > #main_pack > #place-app-root > #place-main-section-root > div > section > div > div.claas > div > div > div.flicking-camera > div.BkqXt > a > div');
        // $에 cheerio를 로드한다.
        ...
        const lists = $("#wrap > #container > #content > #main_pack > #place-app-root > #place-main-section-root > div > section > div > div.claas > div > div > div.flicking-camera > div.BkqXt");
        console.log('lists.length', lists.length);

        // const backgroundImage = await page.evaluate(lists => window.getComputedStyle(lists).backgroundImage, await page.$('.K0PDV _div'));
        ...
        await lists.each(async (index, list) => {
            ...
            // const backgroundImage = await page.evaluate(list => window.getComputedStyle(list).backgroundImage, await page.$('.K0PDV _div'));
            // console.log('backgroundImage', backgroundImage);

            // console.log('list');
            // 각 리스트의 하위 노드중 호텔 이름에 해당하는 요소를 Selector로 가져와 텍스트값을 가져온다.
            // const divTest = $(list).find('a > div').attr('background-image');
            const divTest = $(list).find('a > div').attr('style');
            // const divTest = $(list).find('a > div').children[0];
            // const divTest = $(list).find('a > div').getComputedStyle('background-image');
            // const divTest = $(list).find('a > div');
            // console.log(window.getComputedStyle(list).backgroundImage);
