...
exports.main = async (req, res, next) => {
    try {
        ...
        // 멜론 실시간 차트
        const [melonData, melonFilelist] = await melonFileFunction.melonFile(undefiendVar);
        result['melondata'] = {data: melonData, filelist: melonFilelist};

        // 지니 실시간 차트
        const [genieData, genieFilelist] = await genieFileFunction.genieFile(undefiendVar);
        result["geniedata"] = {data: genieData, filelist: genieFilelist};

        // 유튜브 실시간 차트
        const [youtubeData, youtubeFilelist] = await youtubeFileFunction.youtubeFile(undefiendVar);
